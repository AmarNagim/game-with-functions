...
def alarmText():
    alarmText = input("""Het alarm in de bank gaat af. Het duurt je iets te lang.
...
def getCaughtOrKillExitText():
    getCaughtOrKillExitText = input("""
...
of de agent die voor je vluchtauto staat neerschieten en snel je vluchtauto in gaan. neerschieten/meewerken: """).lower()
    return getCaughtOrKillExitText

def getCaughtOrKillKill():
    getCaughtOrKillKill = print("""
...
werd je neergeschoten door de andere agenten...""")
    return getCaughtOrKillKill

def throwMoneyAwayFirst():
    throwMoneyAwayFirst = input("""Je bent snel de riolering ingekropen vanuit het luik wat in de grond zit.
...
van het geld op de grond en rent snel door. of je zet door met het volle geld bedrag. weggooien/doorzetten: """).lower()
    return throwMoneyAwayFirst

def throwMoneyAwaytext():
    ThrowMoneyAwayText = print("""
De agent is neer! Je rent richting je busje... Net voordat je de deur van je busje open kon trekken 
werd je neergeschoten door de andere agenten...""")
    return ThrowMoneyAwayText

def throwMoneyAwayThrow():
    throwMoneyAwayThrow = input("""
...
of te voet verder te gaan. bellen/lopen: """).lower()
    return throwMoneyAwayThrow

def throwMoneyAwayCall():
    throwMoneyAwayCall = print("""
...
De politie heeft je te pakken gekregen. Je hebt 5 jaar celstraf gekregen.""")
    return throwMoneyAwayCall

def throwMoneyAwayWalk():
    throwMoneyAwayWalk = print("""
...
Je bent weggekomen met 600.000 euro!""")
    return throwMoneyAwayWalk

def throwMoneyAwayDoorzetten():
    throwMoneyAwayDoorzetten = print("""
...
Je bent gepakt. Je hebt 5 jaar celstraf gekregen.""")
    return throwMoneyAwayDoorzetten

def throwMoneyAwayEscape():
    throwMoneyAwayEscape = input("""
...
if vraag_1 == 'wachten':
    exit_or_sewer = sirenesText()
    if exit_or_sewer == 'uitgang':
        getCaughtOrKill = getCaughtOrKillExitText()
        ...
        print('')
        ThrowMoneyAway = throwMoneyAwayFirst()
        if ThrowMoneyAway == 'weggooien':
            call = throwMoneyAwayThrow()
            if call == 'bellen':
                throwMoneyAwayCall()
                print('')
                time.sleep(1)
...
"""Je hebt 25/100 punten, omdat je het hebt overleefd maar bent opgepakt en een celstraf moet uizitten""")
    ...
    choosingTheEasyWay = throwMoneyAwayEscape()
